...
export const saveSearch = (newSession: SearchSession): SearchSession[] => {
    ...
    // Remove potential duplicates by timestamp before adding
    searches = searches.filter(s => s.timestamp !== newSession.timestamp);

    // Add new session to the beginning
    const updatedSearches = [newSession, ...searches];
...
export const exportSearches = (sessionToExport?: SearchSession): void => {
    try {
        ...
        const fileName = sessionToExport
            ? `jjia_export_${sessionToExport.query.substring(0, 15)}_${timestamp}.json`
